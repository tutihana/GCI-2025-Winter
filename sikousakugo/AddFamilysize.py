...
# FamilyCategory はカテゴリとして扱いたいので文字のまま
...
# CatBoost に渡すカテゴリ特徴量の「列インデックス」を取得
cat_feature_names = ['Embarked', 'Rare_name', 'Prefix_Class', 'FamilyCategory','Sex']
cat_features = [X.columns.get_loc(c) for c in cat_feature_names]

# ==========================
# CatBoost モデル構成（Family 最適化版）
# ==========================
model = CatBoostClassifier(
    iterations=400,        # 木の本数：ほどほど
    learning_rate=0.05,    # 少し大きめで早めに打ち切る
    depth=4,               # 木は浅め：過学習しにくい
    l2_leaf_reg=6.0,       # 正則化強め：極端な分岐を抑える
    subsample=0.8,         # 8割サンプリングで汎化性能UP
    random_strength=1.5,   # 分割のランダム性を強めて過学習抑制
