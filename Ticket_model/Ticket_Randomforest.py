...
# ==============================
# カテゴリ変数をダミー変数に変換（One-Hot）
# ==============================

# 文字列のままになっているカテゴリ列
cat_cols = ['Embarked', 'Rare_name', 'Prefix_Class', 'FamilyCategory']

# train の X と test をまとめてから get_dummies して、
# 列のズレが出ないようにする
...
X_train, X_valid, y_train, y_valid = train_test_split(
    X_encoded, y, test_size=0.3, random_state=46, stratify=y
)
...
model = RandomForestClassifier(
    ...
    max_depth=5,       # 深さ制限なし（過学習しそうなら数値を入れてもOK）
    min_samples_split=2,
    min_samples_leaf=1,
    random_state=2025,
    n_jobs=-1             # CPUフル活用
