...
for train_idx, valid_idx in kf.split(X):
    ...
    fold_acc = accuracy_score(y_valid, y_valid_pred)
    fold_scores.append(fold_acc)

    print(f"Fold {fold} の正解率:", round(fold_acc, 4))
    fold += 1

print("=" * 40)
print("Foldごとのスコア:", [round(s, 4) for s in fold_scores])
print("平均スコア (5-Fold CV):", round(np.mean(fold_scores), 4))
print("標準偏差 (ブレの大きさ):", round(np.std(fold_scores), 4))

# ==============================
# 全データで最終モデルを学習してテスト予測
# ==============================
...
# 学習データに対する精度も一応チェック（過学習の感触を見る用）
train_pred = final_model.predict(X)
train_acc = accuracy_score(y, train_pred)
print("全訓練データに対する正解率 (final_model):", round(train_acc, 4))
